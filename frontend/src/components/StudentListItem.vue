...
function nccdClass(level: string | null): string {
  return level
    ? (nccdColour[level] ?? "text-text-secondary")
    : "text-text-secondary";
}
</script>
...
<template>
  <button
    type="button"
    class="w-full text-left flex items-center gap-3 px-4 py-3 transition-colors hover:bg-app-bg focus-visible:outline-none border-l-[3px]"
    :class="selected ? 'bg-[#F5FBFB] border-l-teal' : 'border-l-transparent'"
  ...
  >
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="text-xs mt-0.5" :class="nccdClass(student.nccd_level)">
        NCCD Level: {{ student.nccd_level ?? "Not Recorded" }}
      </p>

      <!-- Primary disability label (secondary text, only when present) -->
      <p
        v-if="student.primary_disability"
