...
**Catching a cross-tenant data leakage bug through thorough testing.** After completing the application, I tested it properly by logging in as users from two different tenant schools within the same browser tab session. I found that staff from Springfield Primary School were visible in the Assign Staff dropdown when logged in as a Riverside Secondary College account — data from one school leaking into another school's session. The backend tenancy scoping was correct; the problem was in the frontend. `useReferenceStore` caches staff and year level data with a `loaded` flag to avoid redundant API calls, but a SPA logout is a JavaScript route change, not a page reload — the store stays in memory. The fix was a `reset()` method on the reference store called from `logout()`, so the cache is always cleared between sessions. Without the multi-tenant test, this would not have been caught — the application would have appeared to work correctly in single-tenant testing.
